...
ROSETTA__OAIPMH_RULES_1_0 = {
    version: "1.0",
    rs_records: {
        records: { "@" => [ lambda { |d,o| 
            rdata = []

            pp "ppppppppppppppppppppppppppppppppppppppppr"
            pp d
            pp "ppppppppppppppppppppppppppppppppppppppppr"

            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_record], d, out, o)
    ...
    rs_record: {
        record: { "$.mets" => lambda { |d,o| 
            ...
            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_data_array], d, out, o)
            rdata = out[:data]
            out.clear

            o[:datePublished] = rdata[:datePublished]

            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_data_has_part], d['structMap'], out, o)
            ...
            rdata[:hasPart].map! { |p|
                ...
                unless input.size != 1
                    rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_datapart_expand], input[0], out, o)
    ...
    rs_data_array: {
        data: { "@.dmdSec[?(@._ID=='ie-dmd')]" => [ lambda {|d,o|
            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_data], d['mdWrap']['xmlData']["record"], out, o)
            out[:data]
        } ]}
    },
    rs_data: {
...
=end                    
            ...
            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_basic_schema], d, out, o)
            rdata.merge!(out[:basic_schema].to_h)
            out.clear

            o[:index] = 0
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_record_data], d, out, o)

            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_in_language], d, out, o)
            
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_is_part_of], d, out, o)
    ...
    rs_data_has_part: {
        hasPart: { "$" => [ lambda {|d,o|
            ...
            # pp o[:name]
            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_data_part], d['div']['div']['div'] , out, o)
            out[:part]
        } ] } 
    },
    rs_data_part: {
    ...
    rs_datapart_expand: {
        datapart_expand: { "$.techMD.mdWrap.xmlData.dnx" => [ lambda {|d,o|
            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_dataparts_section], d, out, o)
    ...
    rs_collection: {
        is_part_of_collection: { "@.amdSec[?(@._ID == 'ie-amd')].techMD.mdWrap.xmlData.dnx.section" =>  lambda { |d,o| 
            out = DataCollector::Output.new
            rules_ng.run(ROSETTA__OAIPMH_RULES_1_0[:rs_collection_section], d, out, o)
            out[:collection]
        }}
    },
